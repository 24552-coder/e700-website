function scanGmailReplies() {
  try {
    // 使用進階 Gmail API，避開原生 GmailApp 的每日額度限制
    var response = Gmail.Users.Messages.list('me', { q: 'is:inbox -is:starred', maxResults: 50 });
    ...
    if (response.messages && response.messages.length > 0) {
      for (var i = 0; i < response.messages.length; i++) {
        ...
        var msgDetail = Gmail.Users.Messages.get('me', msgId);
